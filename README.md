...
* [`IDENTITY.cpy`](src/Cobol/SUPPLIER.cpy) - copybook used to map data passed data back from LinkToSecurity.java.
...
* [`LINK2SUP.cbl`](src/Cobol/LINK2SUP.cbl) - run from terminal to invoke GETSUPPI (LinkToLiberty.java) via Link to Liberty.
* [`LINK2SEC.cbl`](src/Cobol/LINK2SEC.cbl) - run from terminal to invoke L2LSEC (LinkToSecurity.java) via Link to Liberty and display results to the user.
* [`LINK2TXN.cbl`](src/Cobol/LINK2TXN.cbl) - run from terminal to invoke L2LTRAN (LinkToTransaction.java) via Link to Liberty, accepting input ACTION from the terminal and passing to Java in container "ACTION".
